...
public class Interpolation {
	...
	final static float c1 = 1.70158f;
	final static float c2 = c1 * 1.525f;
	final static float c3 = c1 + 1.0f;
	final static float c4 = (float) (2 * Math.PI) / 3.0f;
	final static float c5 = (float) (2 * Math.PI) / 4.5f;
	
	final static float n1 = 7.5625f;
	final static float d1 = 2.75f;
	...
	private static float easeInOutQuad(float _value) 	{ return _value < 0.5f ? 2 * _value * _value : 1 - (float) Math.pow(-2.0 * _value + 2.0, 2.0) / 2; }
	
	private static float easeInCubic(float _value) 		{ return (float) Math.pow(_value, 3.0); }
	private static float easeOutCubic(float _value) 		{ return 1.0f - (float) Math.pow(1.0 - _value, 3.0); }
	private static float easeInOutCubic(float _value) 	{ return _value < 0.5f ? 4.0f * _value * _value * _value : 1.0f - (float) Math.pow(-2.0 * _value + 2.0, 3.0) / 2.0f; }
	
	private static float easeInQuart(float _value) 		{ return (float) Math.pow(_value, 4.0); }
	private static float easeOutQuart(float _value) 		{ return 1.0f - (float) Math.pow(1 - _value, 4.0); }
	private static float easeInOutQuart(float _value) 	{ return _value < 0.5f ? 8 * (float)Math.pow(_value, 4.0) : 1.0f - (float) Math.pow(-2.0 * _value + 2.0, 4.0) / 2.0f; }
	
	private static float easeInQuint(float _value) 		{ return (float) Math.pow(_value, 5); }
	private static float easeOutQuint(float _value) 		{ return 1.0f - (float) Math.pow(1 - _value, 5); }
	private static float easeInOutQuint(float _value) 	{ return _value < 0.5 ? 16 * (float) Math.pow(_value, 5.0f) : 1.0f - (float)Math.pow(-2.0 * _value + 2.0, 5.0) / 2.0f; }
	
	private static float easeInExpo(float _value) 		{ return _value == 0 ? 0 : (float)Math.pow(2, 10 * _value - 10); }
	private static float easeOutExpo(float _value) 		{ return _value == 1 ? 1 : 1 - (float)Math.pow(2, -10 * _value); }
	...
	private static float easeInCirc(float _value) 		{ return 1.0f - (float)Math.sqrt(1.0 - (float)Math.pow(_value, 2.0)); }
	...
	private static float easeOutBack(float _value) 		{ return 1 + c3 * (float)Math.pow(_value - 1, 3) + c1 * (float)Math.pow(_value - 1, 2); }
